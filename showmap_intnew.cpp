...
double sun_radius_deg = ((31.00/2.00)*60.00) / 3600.00;         
int gSunAtZenith=0;
...
void usage()
{
  ...
  printf("\t\t\t\t\tsun_radius_deg=%.2f    - solar radius in degree [default %.2f [deg]]\n",sun_radius_deg,sun_radius_deg);
  printf("\t\t\t\t\tsun_zenith=%d           - if put Sun in zenith (for test purposes only) [default %d]\n",gSunAtZenith,gSunAtZenith);
  ...
  printf("\t\t\t\tuse_maps=%d              - use maps of antenna pattern, sky etc [default %d]\n",gUseMaps,gUseMaps);
  printf("\t\t\t\tconst_tsky=%.2f          - to set a constant T_sky=%.2f [default ANGELICA model]\n",gConstTsky,gConstTsky);
            ...
            case 'q':
                if( optarg ){
                   ...
                   if( pars.GetVarAndValue(envvar) ){
                      ...
                      if( strcmp(envvar.szName.c_str(),"sun_zenith")==0 ){
                         gSunAtZenith = atol(envvar.szValue.c_str());
                      }
                      if( strcmp(envvar.szName.c_str(),"sun_radius_deg")==0 ){
                          sun_radius_deg = atof(envvar.szValue.c_str());
                          sun_radius_arcsec = sun_radius_deg*3600.00;
                      }
                      ...
                      if( strcmp(envvar.szName.c_str(),"use_maps")==0 ){
                         gUseMaps = atol(envvar.szValue.c_str());
                      }
                      if( strcmp(envvar.szName.c_str(),"const_tsky")==0 ){
                         gConstTsky = atof(envvar.szValue.c_str());
                      }
...
CBgFits* ant_pattern_map=NULL;

// loop over angles from theta=90 to -90, phi=0 to 2PI
...
//  double gm_filter = get_gm_transmiss(freq_mhz);
  ...
  if( bAddSun ){
     ...
     if( gSunAtZenith > 0 ){
        azh2radec( 0, 90, unix_time, geo_long, geo_lat, sun_ra, sun_dec );
        sun_az = 0;
        sun_alt = 90;
        bAddSun = 1;
     }
     printf("Sun (RA,DEC) = (%.8f,%.8f) , (AZ,ALT) = (%.8f,%.8f) at uxtime=%.2f at location (%.4f,%.4f) [deg]\n",sun_ra,sun_dec,sun_az,sun_alt,unix_time,geo_long,geo_lat);
  }else{
     printf("WARNING : sun is not included into the model\n");
  }
...
//  for(vector<cSkyInfo>::iterator skyinfo=sky_intensities.begin();skyinfo!=sky_intensities.end();skyinfo++){
  ...
  // WARNING : here theta and phi are azimuthal coordinates (AZIM,ZENDIST) as integration does other way
  // I go over local spherical coordinates and find sky temperature in the Healpix map
  double delta_zendist_arcsec=240; // dOMEGA=0.0000039947 [srad] ~= 412 x 412 arcsec 
  double delta_azim_arcsec=240;

  delta_zendist_arcsec=120;
  delta_azim_arcsec=120;

  double zen_dist_max = 180;

  int fill_maps=0;
  if( gUseMaps > 0 && !ant_pattern_map ){  
     int map_size_x = (zen_dist_max*3600)/delta_zendist_arcsec + 5;
     int map_size_y = (360.00*3600)/delta_azim_arcsec + 5;
     int pixels = (map_size_x*map_size_y);
     printf("Initializing MAPS %d x %d = %d pixels = %.2f MB...",map_size_x,map_size_y,pixels,(pixels*sizeof(float))/1e6);fflush(stdout);
     ant_pattern_map = new CBgFits(map_size_x,map_size_y);
     fill_maps=1;
     printf("done\n");fflush(stdout);
  }else{
     printf("MAPS not required or already initialised (gUseMaps=%d, ant_pattern_map=0x%x)\n",gUseMaps,ant_pattern_map);
  }
  
  printf("Starting integrating loop ...\n");fflush(stdout);
  CMyProgressBar progress(0,180);
  double zen_dist=0; // in ANT coordinate system
  int z=0;
  int n_sun_pixels=0;
  while( zen_dist <= zen_dist_max ){
    printf("zen_dist = %.20f\n",zen_dist);fflush(stdout);
    double azim_deg=0;
    int a=0;
    while( azim_deg <= 360 ){    
       double azim_rad = azim_deg * deg2rad_const;
       double zendist_rad = zen_dist * deg2rad_const;        
       double alt_deg = 90 - zen_dist;

       // source 
       double azim_source_deg = azim_deg;
       double alt_source_deg  = alt_deg;
       double zen_dist_source = zen_dist;


       // TODO : check if azim_deg should not be rotated or is inside this function: 
       double pattern = 0;
       if( alt_deg > 0 ){
          pattern = 1000.00*exp( -(zen_dist*zen_dist)/(gAntennaPattern.m_AntennaBeamWidth*gAntennaPattern.m_AntennaBeamWidth) );
       }
       // double pattern = gAntennaPattern.antenna_pattern_formula( freq_mhz, azim_deg, zen_dist );
/*       if( gUseMaps > 0 ){
          pattern = ant_pattern_map->getXY(a,z);
       }else{
          pattern = gAntennaPattern.antenna_pattern_formula( freq_mhz, azim_deg, zen_dist );
          if( fill_maps > 0 ){
             ant_pattern_map->setXY(a,z,pattern);
          }
       }*/
...
//       }
       ...
       if( gIncludeIonosphere ){
          ...
          if( gIncludeIonoRefraction > 0 ){
             ...
             // antenna beam direction (azim_deg,alt_deg,zen_dist) remain unchanged, but the direction pointing to space is changed:
             zen_dist_source = zen_dist + delta_theta_deg;
             alt_source_deg = (90.00 - zen_dist_source);             
          }       
       }
...
/*       if( gIncludeTropoRefraction > 0 ){
          ...
          alt_deg = (90.00 - zen_dist);
          pattern = gAntennaPattern.antenna_gain_from_azh( freq_mhz, azim_deg, zen_dist );
       } */

       // now alt,azim,zen_dist are corrected for the refraction so I need to find T_sky in this direction 
       // first calculate galactic coordinates from azimuthal ones 
       ...
       // based on pointing of the ray outside the ionosphere (azim_source_deg, alt_source_deg) calculate position (GLON,GLAT) (RA,DEC)
       hor2gal_fromJD( azim_source_deg, alt_source_deg, geo_long, geo_lat, JD_time, glon, glat, ra, dec );
       double phi_deg = glon;
       double theta_deg = 90.00 - glat;
                  

       double theta_rad = theta_deg * deg2rad_const;
       double phi_rad   = phi_deg   * deg2rad_const;
       pointing ptg( theta_rad , phi_rad );
       double intensity = healpix_skymap.interpolated_value( ptg );                  
       if( gConstTsky >= 0 ){
          intensity = gConstTsky;
       }
       double delta_sky = intensity;

       if( fabs(glon)<0.1 && fabs(glat)<0.1 ){
          printf("DEBUG : GAlaxy center (GLON,GLAT)=(%.2f,%.2f) -> (RA,DEC)=(%.2f,%.2f) -> (AZIM,ALT)=(%.2f,%.2f) , intensity=%.8f\n",glon, glat, ra, dec, azim_source_deg, alt_source_deg,intensity);
       }
                         
       if( gOnlyAboveHorizont >= 0 ){
          if( alt_deg < gOnlyAboveHorizont /* && alt>0 TEST with signal from underground added - according to antenna-over-gnd-screen-pattern */ ){
       ...
       double T_sun = 0.00;
       if( bAddSun >0 && alt_deg>0 ){ // if Sun is enabled in the code and pixel Alt>0 :
          double fabs_dec = fabs_inline( dec );
          if( fabs_dec < (26.00+sun_radius_deg) || gSunAtZenith>0 ){ // if close enough from the celestial Equator 
             ...
             if( ra_diff<(5.00+sun_radius_deg) && dec_diff<(5.00+sun_radius_deg) ){
               // only in case we are close to Sun call get_sun_brightness (expensive sin/cos calculations)
               T_sun = get_sun_brightness( ra, dec, azim_deg, alt_deg, sun_ra, sun_dec );
               delta_sky += T_sun;
               n_sun_pixels++;
               // sun_sum += T_sun;
             }
       ...
       if( gIncludeIonosphere ){               
          if( gIncludeIonoAbsorption > 0 ){                 
             // without inlusion of the ionosphere att_num=1
       ...
       double delta = pattern*delta_sky;

       // dOMEGA :
       double delta_azim_rad = (delta_azim_arcsec/3600.00)*deg2rad_const;
       double delta_zendist_rad = (delta_zendist_arcsec/3600.00)*deg2rad_const;
       double d_omega = delta_azim_rad*delta_zendist_rad*sin(zen_dist*deg2rad_const);
                     

       // Calculation of tau_g for the 2015 paper :       
       double tau_g_val = tau_g(zen_dist);
       sum_tau_g += tau_g_val*pattern*d_omega;
       sum_tau_g_bar += tau_g_val*pattern*delta_sky*d_omega;
       ...
       sum += delta*d_omega;    
       antenna_pattern_sum += pattern_for_pattern_sum *d_omega;            
       if( alt_deg < 0 ){
          antenna_pattern_sum_below_hor += pattern_for_pattern_sum *d_omega;       
          sum_below_hor += delta *d_omega;
       }
       if( T_sun > 0 ){
          sun_sum += pattern*ant_gain_num*gm_filter*T_sun*d_omega; // to optimize Sun/No Sun into one 
       }
    
       if( gDebug >= 5 ){
          printf("DEBUG : %.8f %.8f %.8f %.8f %.8f %.8f %.20f %.20f %.20f %.20f %.20f\n",azim_deg,alt_deg,azim_source_deg,alt_source_deg,glon,glat,(double)delta_sky,(double)delta,(double)sum,(double)antenna_pattern_sum,(double)(sum/antenna_pattern_sum));
       }
    
        a++;
        azim_deg += delta_azim_arcsec/3600.00;
     } // end of phi_deg loop
     
     z++;
     zen_dist += delta_zendist_arcsec/3600.00;
     progress.Update(zen_dist);
  }
  ...
  antenna_pattern_sum_below_hor = antenna_pattern_sum_below_hor;
  sum_below_hor = sum_below_hor;
...
void print_parameters()
{
  ...
  printf("\t\tSun radius    = %.2f [deg]\n",sun_radius_deg);
  printf("\t\tSun at zenith = %d\n",gSunAtZenith);
