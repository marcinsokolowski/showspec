...
mystring gInDir="gsm/"; // USED TO BE : "$(BIGHORNS)/software/analysis/gsm/10Mhz_to_500Mhz_step10Mhz/";
...
// MOON :
double moon_T_b = 200.00;
// double moon_radius_arcsec = 31.7;
// double moon_radius_deg = 31.7/60.00;
...
// sensitivity :
int gShowSensitivity=1;
// double gShowSensitivityAzimDeg=(326.25-180); // azimuth in libnova convention from S towards W 
// double gShowSensitivityZaDeg=11.46;
// ch=63 (80MHz obsID=1165141552 , python ./eda_delay.py 14.488870 309.531504 )
double gShowSensitivityAzimDeg=0.00; // (309.531504-180); // azimuth in libnova convention from S towards W 
double gShowSensitivityZaDeg=0.00; // 14.488870;
double gShowSensitivityTrcv=60; // 60 K 
string gShowSensitivityTrcvType=""; // empty string -> use gShowSensitivityTrcv , skala4 -> use SKALA4 values, etc 
...
void print_bighorns_reference();
void print_gsm_reference();
void print_mwa_2016_beam_reference();
...
void usage()
{
  ...
  printf("\n\nIN_FILE_LIST - list of frequencies to be processed (it is located in %s directory, possible options in_list_dat for all frequencies from 10-400 MHz (10 MHz steps) or ni_list_070.00_MHz to generate a specified frequency\n",gInDir.c_str());
  printf("\n\noptions :\n");
  ...
  printf("-t PATTERN_TYPE : antenna pattern type 0-dipol over gnd-screen, 1-isotropic [default 0], 2 - Gaussian Beam exp(-(theta/theta0)^2), use -q ant_beam_width=50 to specify Gaussian Beam Width theta0 [default = %.2f], 3 - USER DEFINED FUNCTION (can be implemented in file user_defined_antenna_pattern.cpp and requires recompilation)\n",gAntennaPattern.m_AntennaBeamWidth);
  ...
  printf("\t\t\t\tMAX_PHI_COUNT=%d         - maximum values in PHI angle [default %d]\n",MAX_PHI_COUNT,MAX_PHI_COUNT);
  printf("\t\t\t\tMAX_THETA_COUNT=%d         - maximum values in THETA angle [default %d]\n",MAX_THETA_COUNT,MAX_THETA_COUNT);
  ...
  printf("\t\t\t\tsun=%d                   - enables/disabled Sun model adding [default %d]\n",gAddSun,gAddSun);
  printf("\t\t\t\tmoon=%d                  - enables/disabled Sun model adding [default %d]\n",gAddMoon,gAddMoon);
  printf("\t\t\t\tmoon_tb=%.2f             - brightness temperature of the Moon [default %.2f K]\n",moon_T_b,moon_T_b);
  ...
  printf("\t\t\t\tsite=%s              - known sites muresk (DEFAULT), mwa, ebo, ebo201309, ebo201312 (Eyre Bird Observatory), muresk (DEFAULT = %s), wond20140405, wond20140406, wond\n",gSiteName.c_str(),gSiteName.c_str());
  ...
  printf("\t\t\t\tcache_on=%d              - enabled caching of skymodel files (.out files), by default [CacheON=%d]\n",gSkyMapCache.GetCacheONOFF(),gSkyMapCache.GetCacheONOFF());
  printf("\t\t\t\tant_cache_on=%d           - enabled caching of antenna pattern, by default [CacheON=%d]\n",gAntennaPattern.m_bCacheON,gAntennaPattern.m_bCacheON);
  printf("\t\t\t\tmax_feko_theta           - maximum value of theta angle simulated in FEKO [default = %.2f deg]\n",CAntPatternParser::m_SimulMaxTheta);
  printf("\t\t\t\tmax_feko_phi             - maximum value of phi angle simulated in FEKO [default = %.2f deg]\n",CAntPatternParser::m_SimulMaxPhi);
  ...
  printf("\t\t\t\tbradley=%d               - specifies that ffe file with antenna patern is in Bradley format : ELEVATION AZIMUTH .... GAIN_LINEAR[not db] , by default = %d\n",CAntPatternParser::m_BradleyVersion,CAntPatternParser::m_BradleyVersion);
  printf("\t\t\t\t\telev_in_antpat=%d      - elevation in antenna pattern (works only in bradley=1 version, by default enabled by bradley=1)\n",CAntPatternParser::m_ElevationInAntPattern);
  printf("\t\t\t\t\tang_in_radians=%d      - angles in radians [default %d]\n",CAntPatternParser::m_AnglesInRadians,CAntPatternParser::m_AnglesInRadians);
  printf("\t\t\t\t\tgain_in_linear=%d      - if gain is in dB (standard FEKO) or linear (Bradley) [default dB]\n",CAntPatternParser::m_GainInLinearScale);
  printf("\t\t\t\t\tgain_column_index=%d   - index of gain column in input pattern file [default %d], starting with 0\n",CAntPatternParser::m_InputFileFormat_GainColIdx,CAntPatternParser::m_InputFileFormat_GainColIdx); 
  printf("\t\t\t\t\ttheta_column_index=%d  - index of gain column in input pattern file [default %d], starting with 0\n",CAntPatternParser::m_InpitFileFormat_ThetaColIdx,CAntPatternParser::m_InpitFileFormat_ThetaColIdx); 
  printf("\t\t\t\t\tphi_column_index=%d    - index of gain column in input pattern file [default %d], starting with 0\n",CAntPatternParser::m_InpitFileFormat_PhiColIdx,CAntPatternParser::m_InpitFileFormat_PhiColIdx); 
  printf("\t\t\t\t\tmwa_beam_type=%d       - MWA beam by default non-MWA beam is used, available options 2016 - uses 2016 model (see option -t for other possible values)\n",CAntPattern::m_AntennaPatternType);
  printf("\t\t\t\t\tmwa_gridpoint=%d       - MWA beam gridpoint [default %d]\n",CAntPattern::gMWAGridpoint,CAntPattern::gMWAGridpoint);
  printf("\t\t\t\t\tshow_sensitivity=%d    - if calculate sensitivity [default %d]\n",gShowSensitivity,gShowSensitivity);
  printf("\t\t\t\t\tshow_sensitivity_azim_deg_nova=%.2f - azimuth to calculate sensitivity at in libnova convention going from South towards West [default %.2f deg]\n",gShowSensitivityAzimDeg,gShowSensitivityAzimDeg);
  printf("\t\t\t\t\tshow_sensitivity_azim_deg=%.2f - azimuth to calculate sensitivity at in standard convention from the North towards East [default %.2f deg]\n",gShowSensitivityAzimDeg,gShowSensitivityAzimDeg);
  printf("\t\t\t\t\tshow_sensitivity_za_deg=%.2f - zenith angle to calculate sensitivity at in standard convention from the North towards East [default %.2f deg]\n",gShowSensitivityZaDeg,gShowSensitivityZaDeg);
  printf("\t\t\t\t\tshow_sensitivity_trcv=%.2f - zenith angle to calculate sensitivity at in standard convention from the North towards East [default %.2f deg]. Possible values SKALA4 (or skala4).\n",gShowSensitivityTrcv,gShowSensitivityTrcv);
  ...
  print_bighorns_reference();
  print_gsm_reference();
  print_mwa_2016_beam_reference();
  
  exit(0);
}

double azimNova2Astro( double azim_nova )
{
   double azim_astro = azim_nova + 180.00;
   if( azim_astro >= 360.00 ){
      azim_astro = azim_astro - 360.00;
   }
            ...
            case 'q':
                if( optarg ){
                   ...
                   if( pars.GetVarAndValue(envvar) ){
                      ...
                      if( strcmp(envvar.szName.c_str(),"MAX_PHI_COUNT")==0 ){
                         MAX_PHI_COUNT = atol(envvar.szValue.c_str());
                      }
                      if( strcmp(envvar.szName.c_str(),"MAX_THETA_COUNT")==0 ){
                         MAX_THETA_COUNT = atol(envvar.szValue.c_str());
                      }
                      if( strcmp(envvar.szName.c_str(),"sun")==0 ){
                         gAddSun = atol(envvar.szValue.c_str());
                      }
                      if( strcmp(envvar.szName.c_str(),"moon")==0 ){
                         gAddMoon = atol(envvar.szValue.c_str());
                      }
                      if( strcmp(envvar.szName.c_str(),"moon_t_b")==0 ){
                         moon_T_b = atof( envvar.szValue.c_str() );
                      }
                      ...
                      if( strcmp(envvar.szName.c_str(),"max_feko_phi")==0 ){
                         CAntPatternParser::m_SimulMaxPhi = atof(envvar.szValue.c_str());
                      }
                      ...
                      // SPECIAL OPTIONS :
                      if( strcmp(envvar.szName.c_str(),"bradley")==0 ){
                         CAntPatternParser::m_BradleyVersion = atol(envvar.szValue.c_str());
                         CAntPatternParser::m_ElevationInAntPattern = 0; // Bradley writes (AZIM,ZA) in degrees now
                         CAntPatternParser::m_AnglesInRadians = 0; // Bradley writes in degrees
                         CAntPatternParser::m_GainInLinearScale=1;
                      }
                      // printf("\t\t\t\telev_in_antpat=%d        - elevation in antenna pattern (works only in bradley=1 version, by default enabled by bradley=1)\n",CAntPatternParser::m_ElevationInAntPattern);
                      if( strcmp(envvar.szName.c_str(),"elev_in_antpat")==0 ){
                         CAntPatternParser::m_ElevationInAntPattern = atol(envvar.szValue.c_str());
                      }
                      // printf("\t\t\t\t\tang_in_radians=%d      - angles in radians [default %d]\n",CAntPatternParser::m_AnglesInRadians);
                      if( strcmp(envvar.szName.c_str(),"ang_in_radians")==0 ){
                         CAntPatternParser::m_AnglesInRadians = atol(envvar.szValue.c_str());
                      }
                      // printf("\t\t\t\t\tgain_in_linear=%d      - if gain is in dB (standard FEKO) or linear (Bradley) [default dB]\n",CAntPatternParser::m_GainInLinearScale);
                      if( strcmp(envvar.szName.c_str(),"gain_in_linear")==0 ){
                         CAntPatternParser::m_GainInLinearScale = atol(envvar.szValue.c_str());
                      }
                      
                      // printf("\t\t\t\t\tgain_column_index=%d   - index of gain column in input pattern file [default %d]\n",CAntPatternParser::m_InputFileFormat_GainColIdx,CAntPatternParser::m_InputFileFormat_GainColIdx);
                      if( strcmp(envvar.szName.c_str(),"gain_column_index")==0 ){
                         CAntPatternParser::m_InputFileFormat_GainColIdx = atol( envvar.szValue.c_str() );
                      }
                      if( strcmp(envvar.szName.c_str(),"theta_column_index")==0 ){
                         CAntPatternParser::m_InpitFileFormat_ThetaColIdx = atol( envvar.szValue.c_str() );
                      }
                      if( strcmp(envvar.szName.c_str(),"phi_column_index")==0 ){
                         CAntPatternParser::m_InpitFileFormat_PhiColIdx = atol( envvar.szValue.c_str() );
                      }
                      
                      // printf("\t\t\t\t\tmwa_beam_type=%d       - MWA beam by default non-MWA beam is used, available options 2016 - uses 2016 model\n",gMWABeamType);
                      if( strcmp(envvar.szName.c_str(),"mwa_beam_type")==0 ){
                          CAntPattern::m_AntennaPatternType = (eAntennaPatternType)atol( envvar.szValue.c_str() );
                          CAntPattern::gAntennaPatternFormula = 1;
                      }
                      // printf("\t\t\t\t\tmwa_gridpoint=%d       - MWA beam gridpoint [default %d]\n",gMWAGridpoint,gMWAGridpoint);
                      if( strcmp(envvar.szName.c_str(),"mwa_gridpoint")==0 ){
                         CAntPattern::gMWAGridpoint = atol( envvar.szValue.c_str() );
                      }
                      
                      if( strcmp(envvar.szName.c_str(),"show_sensitivity")==0 ){
                         gShowSensitivity = atol( envvar.szValue.c_str() );
                      }
                      
                      //  printf("\t\t\t\t\tshow_sensitivity_azim_deg_nova=%.2f - azimuth to calculate sensitivity at in libnova convention going from South towards West [default %.2f deg]\n",gShowSensitivityAzimDeg,gShowSensitivityAzimDeg);
//  printf("\t\t\t\t\tshow_sensitivity_azim_deg=%.2f - azimuth to calculate sensitivity at in standard convention from the North towards East [default %.2f deg]\n",gShowSensitivityAzimDeg,gShowSensitivityAzimDeg);
//  printf("\t\t\t\t\tshow_sensitivity_za_deg=%.2f - zenith angle to calculate sensitivity at in standard convention from the North towards East [default %.2f deg]\n",gShowSensitivityZaDeg,gShowSensitivityZaDeg);
                      if( strcmp(envvar.szName.c_str(),"show_sensitivity_azim_deg_nova")==0 ){
                         gShowSensitivityAzimDeg = atof(envvar.szValue.c_str());
                      }
                      if( strcmp(envvar.szName.c_str(),"show_sensitivity_azim_deg")==0 ){
                         double azim_from_north = atof(envvar.szValue.c_str());
                         
                         // convert to azim in Nova convention from South -> West 
                         gShowSensitivityAzimDeg = azim_from_north - 180.00;
                         if ( gShowSensitivityAzimDeg < 0 ){
                            gShowSensitivityAzimDeg = 360.00 + gShowSensitivityAzimDeg;
                         }
                      }
                      
                      if( strcmp(envvar.szName.c_str(),"show_sensitivity_za_deg")==0 ){
                         gShowSensitivityZaDeg = atof(envvar.szValue.c_str());
                      }

// printf("\t\t\t\t\tshow_sensitivity_trcv=%.2f - zenith angle to calculate sensitivity at in standard convention from the North towards East [default %.2f deg]. Possible values SKALA4 (or skala4).\n",gShowSensitivityTrcv,gShowSensitivityTrcv);
                      if( strcmp(envvar.szName.c_str(),"show_sensitivity_trcv")==0 ){
                         if( is_number(envvar.szValue.c_str()) ){
                            double tmp_value = atof( envvar.szValue.c_str() );
                            gShowSensitivityTrcv = tmp_value; 
                            gShowSensitivityTrcvType = "";
                         }else{
                            gShowSensitivityTrcvType = envvar.szValue.c_str();
                            gShowSensitivityTrcv = -1;
                         }
                      }
...
int read_sources( const char* infile )
{
   ...
   while( pLine = ifile.GetLine( TRUE ) ){
      ...
      if( items.size() >= 9 ){
         source.flux_index = atof(items[8].c_str());
      }
      if( items.size() >= 10 ){
         source.flux_ref_freq_mhz = atof(items[9].c_str());
      }
      
      gObjectList.push_back(source);
   }
   
   printf("Read %d sources from file %s\n",(int)gObjectList.size(),infile);
   for(int i=0;i<gObjectList.size();i++){
      cObjectInfo& source = gObjectList[i];
   
      printf("%s : (%.4f,%.4f) [deg] , ( %.8f +/- %.8f Jy ) * (freq_mhz/%.4f)^%.2f, size = %.4f [deg], period = %.2f [ms], pulse_length = %.2f [ms]\n",
                source.name.c_str(),source.ra,source.dec,source.flux,source.err_flux,source.flux_ref_freq_mhz,source.flux_index,source.radius_arcsec/3600.0,source.period*1000.,source.pulse_length*1000.0);
   }
...
inline double get_moon_brightness( double ra, double dec, double azim, double alt, // coordinates of integrated sky point 
                                  double moon_ra, double moon_dec, double moon_radius_deg      // Sun coordinates for current moment 
                                )
{
   if( alt > 0 ){
//      double dist_to_moon = get_ang_distance_deg( ra, dec, moon_ra, moon_dec );   

      // local calculation of Sun distance to make it faster :
      double cos_x = sin(dec*deg2rad_const)*sin(moon_dec*deg2rad_const) + cos(dec*deg2rad_const)*cos(moon_dec*deg2rad_const)*cos( (ra-moon_ra)*deg2rad_const );
      double x = acos( cos_x );
      double dist_to_moon = x * rad2deg_const;
            
      
      // double local_moon_radius_deg = get_moon_ang_size_arcsec( jd );
      if( dist_to_moon < moon_radius_deg ){
         printf("MOON : (ra,dec) = (%.4f,%.4f) [deg], (azim,alt) = (%.4f,%.4f) [deg] vs. moon (ra,dec)=(%.4f,%.4f) -> returning moon temperature = %.2f K\n",ra,dec,azim,alt,moon_ra,moon_dec,moon_T_b);
         return moon_T_b;
      }
   }
...
{
  printf("DEBUG : integrate_galactic_spectrum\n");
...
/*  int freq_index = -1;
  ...
  int bAddMoon = gAddMoon;
  double moon_ra, moon_dec, moon_az, moon_alt, phase, phase_angle, moon_radius_deg;
  if( bAddMoon ){
     get_moon_info( unix_time, geo_long, geo_lat, moon_ra, moon_dec, moon_az, moon_alt, phase, phase_angle, moon_radius_deg );
  ...
  if( gSaveSkyMapFits > 0 && (gSaveSkyMapAtFreq<0 || fabs(gSaveSkyMapAtFreq-freq_mhz)<1 ) ){
     ...
     p_zenith_projection_ant->SetValue(0);
...
/* I am testing alternative way to add point sources in the very end as a separate loop
     ...
     printf("took %d sec\n",(int)(get_dttm()-start));fflush(stdout);
  } 
  
  // get antenna pattern array (map) :
  time_t start = get_dttm();
  double* antenna_pattern_array = NULL;  
  if( gIncludeIonoRefraction <= 0 && CAntPattern::m_AntennaPatternType!=eMWA2016 ){
//  if( gIncludeIonoRefraction <= 0 ){
     ...
     printf("WARNING : ionospheric refraction is turned on (-q refraction=1) or using MWA2016 beam model - antenna pattern is calculated for every single point later in the code\n");
  }
...
//    dec  = skyinfo->dec;
    ...
    if( fabs(skyinfo->glon)<0.1 && fabs(skyinfo->glat)<0.1 ){
       printf("DEBUG : GAlaxy center (GLON,GLAT)=(%.2f,%.2f) -> (RA,DEC)=(%.2f,%.2f) -> (AZIM,ALT)=(%.2f,%.2f) , intensity = %.4f\n",skyinfo->glon, skyinfo->glat, skyinfo->ra, skyinfo->dec, skyinfo->azim, skyinfo->alt, skyinfo->intensity);
    }
#endif
...
//          delta_sky = 0.00;
    ...
    if( gDebug > 0 ){
       if( (npix % 1000) == 0 ){
          printf("npix = %d / %d = %.2f %%...\n",npix,(int)sky_intensities.size(),(double(npix)/double(sky_intensities.size()))*100.00);fflush(stdout);
       }
    }
#endif
    ...
    if( bAddMoon >0 && skyinfo->alt>0 ){ // if Sun is enabled in the code and pixel Alt>0 :
       double ra_diff = fabs_inline(skyinfo->ra - moon_ra);
       double dec_diff = fabs_inline(skyinfo->dec - moon_dec);
          
       if( ra_diff<5.00 && dec_diff<5.00 ){
          // only in case we are close to Sun call get_sun_brightness (expensive sin/cos calculations)
          double T_moon = get_moon_brightness( skyinfo->ra, skyinfo->dec, skyinfo->azim, skyinfo->alt, moon_ra, moon_dec, moon_radius_deg );
          delta_sky = T_moon;
          // sun_sum += T_sun;
       }
    }
    ...
    // Test code for Bradley - adding Crab nabula :
    if( gObjectList.size() ){  
       double T_point_sources=0.00;
       for(int i=0;i<gObjectList.size();i++){
          cObjectInfo& object = gObjectList[i];

          double dist_deg = get_ang_distance_deg( object.ra, object.dec, skyinfo->ra, skyinfo->dec );
          double dist_deg_arcsec = dist_deg*3600.00;
          
          if( dist_deg_arcsec < object.radius_arcsec ){        
             ...
             double z = (90.00 - skyinfo->alt);
             double gain = gAntennaPattern.antenna_gain_from_azh( freq_mhz, skyinfo->azim, z );
             double A_eff = calc_A_eff( freq_mhz, gain );
             double beam_size_srad = (4*M_PI)/gain;
             double beam_size_deg = sqrt(beam_size_srad)*(180.00/M_PI);
             //        double lamda = 300.00 / freq_mhz;
             //        double a_eff = ((lamda*lamda)/(4*M_PI))*gain;        
             double flux = object.flux * power( (freq_mhz/ object.flux_ref_freq_mhz) , object.flux_index ); 
             double object_radius_rad = (object.radius_arcsec/3600.00)*(M_PI/180.00);
             double source_dOMEGA = M_PI*(object_radius_rad*object_radius_rad);
             double source_intensity = flux / source_dOMEGA;
//             double delta_temp = jy2kelvin_gain( source_intensity, freq_mhz, gain ); // was flux (which is integrated flux and is wrong and should be replaced by source specific intensity)
             double delta_temp = jy2brigthnesstemp( source_intensity, freq_mhz ); // was flux (which is integrated flux and is wrong and should be replaced by source specific intensity)
             if( gDebug>0 ){
                printf("DEBUG : flux=%.2f Jy -> I_v = %.2f [Jy/srad] -> T_b(%.2f MHz) = %.8f K (gain=%.4f, A_eff=%.2f m^2, beam_size=%.2f [deg])\n",flux,source_intensity,freq_mhz,delta_temp,gain,A_eff,beam_size_deg);
             }
...
// actually every integration should be T_ant = (T_int_start + T_int_end)/2.00 - or rather calculate for (t_start + inttime/2)
            printf("\t\tObject %s added replacing delta_sky = %.2f K with %.2f K\n",object.name.c_str(),delta_sky,delta_temp);
            delta_sky = delta_temp; // replacing whatever is there with brightness temperature of the object
...
//      if( fabs(zen_dist-32.24)<=0.1 && fabs(phi_deg-151.88)<=0.1 ){
//         printf("odo");
//      }
      
      // 20170915 when implementing FEE MWA model : was phi_deg -> skyinfo->azim
      if (zen_dist <= 90 ){
         pattern = gAntennaPattern.antenna_pattern_formula( freq_mhz, skyinfo->azim, zen_dist );      
      }
      // printf("Values : %.4f vs. %.4f\n",skyinfo->azim,phi_deg);
      // pattern = gAntennaPattern.antenna_pattern_formula( freq_mhz, phi_deg, zen_dist );
   }
//   printf("DEBUG : (theta,phi) = (%.2f,%.2f) , pattern = %.8f\n",zen_dist,phi_deg,pattern);
...
//    }
    ...
    if( p_zenith_projection && zen_dist<=90 ){ // only above horizon 
...
/*          if( fabs(x_int-400)<10 && fabs(y_int-400)<10 ){
             printf("(%d,%d) : EQ:(%.4f,%.4f) GAL:(%.4f,%.4f) HOR:(%.4f,%.4f) t = %.2f K\n",(int)x_int,(int)y_int,skyinfo->ra,skyinfo->dec,phi_deg,theta_deg,skyinfo->azim,skyinfo->alt,skyinfo->intensity);
          }*/
       }
//       }
    }
...
//    cout << line << endl;
  ...
  printf("Pure loop took %d [sec] , uxtime = %d\n",(int)(get_dttm()-start),(int)get_dttm());
  if( gIncludeIonosphere ){
     printf("Min/max ionospheric attenuation = %.8f / %.8f\n",min_iono_att_db,max_iono_att_db);
  }
  
  if( gShowSensitivity ){
     // gShowSensitivityAzimDeg , gShowSensitivityZaDeg
     double gain = gAntennaPattern.antenna_gain_from_azh( freq_mhz, gShowSensitivityAzimDeg, gShowSensitivityZaDeg, 1 ); // 1->0 uncomment TEST 
     printf("DEBUF gain = %.2f\n",gain);
//     gain = gain / ( antenna_pattern_sum / (4*M_PI) ); - this is already done inside gAntennaPattern.antenna_gain_from_azh 
     double ant_efficiency=1.00;
     double aeff = (7161.97/(freq_mhz*freq_mhz))*(gain*ant_efficiency); // 7161.97 = (lambda^2)/(4PI) - when labmda -> freq , freq-> freq_mhz and a lot cancels out and only stays 7161.97/freq_mhz^2
     double t_ant = sum/antenna_pattern_sum;
     double T_rcv= gShowSensitivityTrcv;// K

/*     if( strlen(gShowSensitivityTrcvType.c_str()) > 0 ){
        if( strcasecmp(gShowSensitivityTrcvType.c_str(),"SKALA4")==0 ){
           T_rcv = get_skala4al_trcv( freq_mhz , eSKALA4AL_Trcv_Ant );
           printf("T_rcv( %.2f MHz) = %.2f K - calculated for SKALA4 antenna\n",freq_mhz,T_rcv);
        }        
     }*/
     
     if( T_rcv < 0 ){
        printf("ERROR : T_rcv < 0 -> setting to default value = 60 K\n");
        T_rcv = 60;
     }
     
     double T_sys = ( t_ant + T_rcv );
     double sens = aeff / ( T_sys );



// TEST :
     // neeeds 0 in last parameter of call to gAntennaPattern.antenna_gain_from_azh :
//     sens = 4*M_PI*(7161.97/(freq_mhz*freq_mhz))*(gain/sum);
     
     printf("sens = 4*M_PI*(7161.97/(freq_mhz*freq_mhz))*(gain/sum) = %.8f x (%.8f / %.8f)\n",(4*M_PI*(7161.97/(freq_mhz*freq_mhz))),gain,(double)sum);
     
     printf("Sensitivity (%.2f MHz) : T_ant = %.2f  = (%.8f / %.8f) , beam(%.4f,%.4f)=%.8f , gain=%.8f , aeff=%.8f, sensitivity=%.20f T_rcv=%.2f\n",
        freq_mhz,t_ant,(double)sum,(double)antenna_pattern_sum,gShowSensitivityAzimDeg,gShowSensitivityZaDeg,gain,gain,aeff,sens,T_rcv);
     printf("SEFD = %.4f [Jy]\n",(2.0*1380.0/sens));
  }
...
//  prepare_Healpix_fitsmap(out_fits,
     ...
     if( p_zenith_projection ){
        ...
        char szUxTime[128],szFreq[64];
        sprintf(szUxTime,"%d",(int)unix_time);
        sprintf(szFreq,"%.2fMHz",freq_mhz);
        
        szOutFits << gOutPutDir.c_str() << "/" << gZenithProjBaseName.c_str() << "_" << szUxTime << "_" << szFreq << ".fits";        
        ...
        szOutFits="";
        szOutFits << gOutPutDir.c_str() << "/" << gZenithProjBaseName.c_str() << "_" << szUxTime << "_" << szFreq << "_ant.fits";
        printf("Saving ANT zenith projection to fits file %s ...\n",szOutFits.c_str());fflush(stdout);
        p_zenith_projection_ant->WriteFits(szOutFits.c_str());

        szOutFits="";
        szOutFits << gOutPutDir.c_str() << "/" << gZenithProjBaseName.c_str() << "_" << szUxTime << "_" << szFreq << "_mult.fits";
...
void print_parameters()
{
  ...
  if( strlen(CAntPattern::m_AntennaPatternFile.c_str())==0 || CAntPattern::gAntennaPatternFormula>0 ){
     ...
     if( gAntennaPattern.m_AntennaPatternType == eMWA2016 ){
        szFormulaName="MWA_FEE_2016_MODEL";
     }
  ...
  printf("Include Moon model          = %d (T_moon = %.2f K)\n",gAddMoon,moon_T_b);
  ...
  printf("Maximum value of phi (simulated in FEKO)   = %.2f [deg]\n",CAntPatternParser::m_SimulMaxPhi);
  ...
  printf("Bradley format              = %d (elevation=%d , in_radians=%d)\n",CAntPatternParser::m_BradleyVersion,CAntPatternParser::m_ElevationInAntPattern,CAntPatternParser::m_AnglesInRadians);
  printf("Gain in linear scale        = %d\n",CAntPatternParser::m_GainInLinearScale);
  printf("# of points in THETA / PHI  = %d / %d\n",MAX_THETA_COUNT,MAX_PHI_COUNT);
  printf("Gain column index (starting from 0)  = %d\n",CAntPatternParser::m_InputFileFormat_GainColIdx);
  printf("Theta column index (starting from 0) = %d\n",CAntPatternParser::m_InpitFileFormat_ThetaColIdx);
  printf("Phi column index (starting from 0)   = %d\n",CAntPatternParser::m_InpitFileFormat_PhiColIdx);
  printf("MWA beam type                        = %d\n",CAntPattern::m_AntennaPatternType);
  printf("gMWAGridpoint                        = %d\n",CAntPattern::gMWAGridpoint);
  printf("Calculate sensitivity                = %d at (azim_nova,za) = (%.4f,%.4f) [deg] , (azim_mwa,za) = (%.4f,%.4f) [deg]\n",gShowSensitivity,gShowSensitivityAzimDeg,gShowSensitivityZaDeg,azimNova2Astro(gShowSensitivityAzimDeg),gShowSensitivityZaDeg);
  printf("T_rcv (for sensitivity)              = %.2f (using type = |%s|)\n",gShowSensitivityTrcv,gShowSensitivityTrcvType.c_str());  
  printf("#################################################\n");
}
...
int main(int argc,char* argv[])
{
  ...
  if( strlen(gOutPutDir.c_str()) > 0 ){
    ...
    szMkDir += gOutPutDir;
    printf("command : %s\n",szMkDir.c_str());
    int ret_val = system((const char*)szMkDir.c_str());
  }
  ...
  if( ni_list.size() <= 0 ){
     printf("Error no frequency was found in specified frequency range %.2f - %.2f MHz\n",gFreqStart,gFreqEnd);
     exit(-1);
  }
  printf("Initialising antenna pattern for %d frequencies, initial pattern map size = (%d x %d) and frequency = %.2f MHz\n",(int)ni_list.size(),MAX_PHI_COUNT,MAX_THETA_COUNT,ni_list[0]);
  
  // initialization of antenna pattern structure :
  gAntennaPattern.Init( ni_list, MAX_PHI_COUNT, MAX_THETA_COUNT, ni_list[0] );  // use first frequency (just in case the only one)
...
//  sprintf(tmp_str,"%s_%s.ang",outfile.c_str(),szUT.c_str());
  ...
  if( gObjectList.size() && 0 ){  // this is not supposed to be here - but inside the main loop near the Sun contribution ?
     double T_point_sources=0.00;
     for(int i=0;i<gObjectList.size();i++){
        cObjectInfo& object = gObjectList[i];
...
double integrate_galactic_spectrum_FAST( const char* infile , double unix_time, ofstream* ofile, double& out_pattern_sum, double& out_sun_sum, int first_freq  )
{
  printf("DEBUG : integrate_galactic_spectrum_FAST\n");
  // uses or not cache - depending on cache_on=1 or =0 parameter (by default disabled)
  vector<cSkyInfo>* pSkyIntensities = gSkyMapCache.get_skyintensities( infile );  
  if( !pSkyIntensities ){
     printf("ERROR : could not get skyintensities from in-file = %s\n",infile);
...
//          delta_sky = 0.00;
    ...
    if( bAddMoon >0 && alt>0 ){ // if Sun is enabled in the code and pixel Alt>0 :
       double ra_diff = fabs_inline(ra - moon_ra);
       double dec_diff = fabs_inline(dec - moon_dec);
          
       if( ra_diff<5.00 && dec_diff<5.00 ){
          // only in case we are close to Sun call get_sun_brightness (expensive sin/cos calculations)
          double T_moon = get_moon_brightness( ra, dec, azim, alt, moon_ra, moon_dec, moon_radius_deg );
          delta_sky = T_moon;
          // sun_sum += T_sun;
       }
    }
    ...
    if( gObjectList.size() ){  
       ...
       for(int i=0;i<gObjectList.size();i++){
          ...
          double dist_deg = get_ang_distance_deg( object.ra, object.dec, ra, dec );
          double dist_deg_arcsec = dist_deg*3600.00;
          
          if( dist_deg_arcsec < object.radius_arcsec ){        
             // calculate (az,h) coordinates from (ra,dec) -> antenna gain in this direction -> antenna effective area -> received flux -> contribution to antenna efficiency :

             double gain = gAntennaPattern.antenna_gain_from_azh( freq_mhz, azim, zen_dist );
             //        double lamda = 300.00 / freq_mhz;
             //        double a_eff = ((lamda*lamda)/(4*M_PI))*gain;        
             double flux = object.flux * power( (150/freq_mhz) , object.flux_index ); 
             double delta_temp = jy2kelvin_gain( flux, freq_mhz, gain );        

        // adding extra temperature from this point source :
// TODO : the extra signal should be weighted by length of the signal vs integration time :
// delta_temp should be a flux averaged over integration time !
// actually every integration should be T_ant = (T_int_start + T_int_end)/2.00 - or rather calculate for (t_start + inttime/2)
            delta_sky += delta_temp; 
//          T_point_sources += delta_temp;
          }
       }
    }
...
//    double pattern = antenna_pattern_array[npix_global];
    double pattern = 0;
    pattern = gAntennaPattern.antenna_pattern( freq_mhz, pPatternFreq, pPatternFreqNext, azim, zen_dist );
    double delta = pattern*ant_gain_num*gm_filter*delta_sky;
    sum += delta;    
...
void print_bighorns_reference()
{
   printf("\n\nIf you used the showspec software for your publication please reference this paper : http://adsabs.harvard.edu/abs/2015PASA...32....4S\n");

   printf("\t@ARTICLE{2015PASA...32....4S,\n");
   printf("\t\tauthor = {{Sokolowski}, M. and {Tremblay}, S.~E. and {Wayth}, R.~B. and \n");
   printf("\t\t{Tingay}, S.~J. and {Clarke}, N. and {Roberts}, P. and {Waterson}, M. and \n");
   printf("\t\t{Ekers}, R.~D. and {Hall}, P. and {Lewis}, M. and {Mossammaparast}, M. and \n");
   printf("\t\t{Padhi}, S. and {Schlagenhaufer}, F. and {Sutinjo}, A. and {Tickner}, J.},\n");
   printf("\t\ttitle = \"{BIGHORNS - Broadband Instrument for Global HydrOgen ReioNisation Signal}\",\n");
   printf("\t\tjournal = {\\pasa},\n");
   printf("\t\tarchivePrefix = \"arXiv\",\n");
   printf("\t\teprint = {1501.02922},\n");
   printf("\t\tprimaryClass = \"astro-ph.IM\",\n");
   printf("\t\tkeywords = {cosmology: observations, dark ages, reionisation, first stars, instrumentation: miscellaneous, methods: data analysis, methods: observational},\n");
   printf("\t\tyear = 2015,\n");
   printf("\t\tmonth = feb,\n");
   printf("\t\tvolume = 32,\n");
   printf("\t\teid = {e004},\n");
   printf("\t\tpages = {e004},\n");
   printf("\t\tdoi = {10.1017/pasa.2015.3},\n");
   printf("\t\tadsurl = {http://adsabs.harvard.edu/abs/2015PASA...32....4S},\n");
   printf("\t\tadsnote = {Provided by the SAO/NASA Astrophysics Data System}\n");
   printf("\t}\n");
}

void print_gsm_reference()
{
   printf("\n\nIf you used Global Sky Model (GSM) software gsm/skybkg_angelica_range , please reference this paper : http://adsabs.harvard.edu/abs/2008MNRAS.388..247D and possibly check webpage : http://space.mit.edu/home/angelica/gsm/\n");
   
   printf("\t@ARTICLE{2008MNRAS.388..247D,\n");
   printf("\t\tauthor = {{de Oliveira-Costa}, A. and {Tegmark}, M. and {Gaensler}, B.~M. and {Jonas}, J. and {Landecker}, T.~L. and {Reich}, P.},\n");
   printf("\t\ttitle = \"{A model of diffuse Galactic radio emission from 10 MHz to 100 GHz}\",\n");
   printf("\t\tjournal = {\\mnras},\n");
   printf("\t\tarchivePrefix = \"arXiv\",\n");
   printf("\t\teprint = {0802.1525},\n");
   printf("\t\tkeywords = {methods: data analysis , astronomical data bases: miscellaneous , ISM: structure , diffuse radiation , radio continuum: ISM},\n");
   printf("\t\tyear = 2008,\n");
   printf("\t\tmonth = jul,\n");
   printf("\t\tvolume = 388,\n");
   printf("\t\tpages = {247-260},\n");
   printf("\t\tdoi = {10.1111/j.1365-2966.2008.13376.x},\n");
   printf("\t\tadsurl = {http://adsabs.harvard.edu/abs/2008MNRAS.388..247D},\n");
   printf("\t\tadsnote = {Provided by the SAO/NASA Astrophysics Data System}\n");
   printf("\t}\n");   
}
...
void print_mwa_2016_beam_reference()
{
   printf("\n\nIf you are used MWA 2016 Full Embedded Element Primary Beam Model, please reference this paper (should be published in PASA by the end of 2017) : http://adsabs.harvard.edu/abs/2017arXiv171007478S\n");
   printf("\t\tTitle = \"Calibration and Stokes Imaging with Full Embedded Element Primary Beam Model for the Murchison Widefield Array\", PASA, 2017\n");
}
